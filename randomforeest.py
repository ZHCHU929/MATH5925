...
for _ in range(simulations):
    ...
    # Fit the model
    model = RandomForestRegressor(n_estimators=20, random_state=_) 
...
print(f"Train RMSE(RF): {avg_rmse_train:.3f}")
print(f"Test RMSE(RF): {avg_rmse_test:.3f}")
print(f"Train MAPE(RF): {avg_mape_train:.3f}%")
print(f"Test MAPE(RF): {avg_mape_test:.3f}%")
print(f"Train RMSPE(RF): {avg_rmspe_train:.3f}%")
print(f"Test RMSPE(RF): {avg_rmspe_test:.3f}%")
